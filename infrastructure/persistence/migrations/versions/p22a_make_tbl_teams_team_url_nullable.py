...
def upgrade() -> None:
    op.execute(sa.text(
        "ALTER TABLE sch_shared.tbl_teams ALTER COLUMN team_url DROP NOT NULL"
    ))


def downgrade() -> None:
    op.execute(sa.text(
        "UPDATE sch_shared.tbl_teams SET team_url = '' WHERE team_url IS NULL"
    ))
    op.execute(sa.text(
        "ALTER TABLE sch_shared.tbl_teams ALTER COLUMN team_url SET NOT NULL"
    ))
