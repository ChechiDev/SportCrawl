"""ORM model for tbl_team_urls in sch_fbref_backend schema."""
...
class TeamUrl(BackendBase):
    """ORM model for sch_fbref_backend.tbl_team_urls.

    Tracks scraping URLs per team with scheduling metadata.
    One row per (team, url_type) combination.
    """
    ...
    __table_args__ = (
        UniqueConstraint("fk_team", "url_type", name="uq_team_urls_team_url_type"),
        CheckConstraint("priority BETWEEN 1 AND 10", name="ck_team_urls_priority"),
        CheckConstraint("cadence_hours > 0", name="ck_team_urls_cadence_hours"),
        CheckConstraint("retry_count >= 0", name="ck_team_urls_retry_count"),
        Index(
            "ix_team_urls_due",
            "next_scrape_at",
            "priority",
        ...
        Index("ix_team_urls_fk_team", "fk_team"),
        Index("ix_team_urls_status", "status"),
