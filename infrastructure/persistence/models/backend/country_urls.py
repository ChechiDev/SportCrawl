"""ORM model for tbl_country_urls in sch_fbref_backend schema."""
...
class CountryUrl(BackendBase):
    """ORM model for sch_fbref_backend.tbl_country_urls.

    Tracks scraping URLs per country with scheduling metadata.
    url_type is restricted to ('country', 'players_list').
    One row per (country, url_type) combination.
    """

    __tablename__ = "tbl_country_urls"
    ...
    __table_args__ = (
        ...
        CheckConstraint("priority BETWEEN 1 AND 10", name="ck_country_urls_priority"),
        CheckConstraint("cadence_hours > 0", name="ck_country_urls_cadence_hours"),
        CheckConstraint("retry_count >= 0", name="ck_country_urls_retry_count"),
        Index(
            "ix_country_urls_due",
            "next_scrape_at",
            "priority",
        ...
        Index("ix_country_urls_fk_country", "fk_country"),
        Index("ix_country_urls_status", "status"),
